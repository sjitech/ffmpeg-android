...
echo ---------------config libvpx [armv5]--------------------
./configure --target=armv5te-android-gcc --disable-examples --disable-docs --enable-static --enable-pic --disable-realtime-only \
	--sdk-path=$ANDROID_NDK_ROOT --prefix=./qj_armv5 \
	|| exit 1

echo ---------------make libvpx [armv5]--------------------
make clean
make all || exit 1
make install || exit 1

cp -fv ./libvpx.a ./qj_armv5/lib/libvpx.a | exit 1
