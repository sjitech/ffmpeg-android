...
echo ---------------config libx264 [armv5]--------------------
./configure --host=armv5-linux-androideabi --enable-static --disable-cli --enable-pic --prefix=./qj_armv5 \
	|| exit 1

export PATH=".:$PATH" #let Makefile use system's perl

echo ---------------make libx264 [armv5]--------------------
make clean
make all || exit 1
make install || exit 1

cp -fv ./libx264.a ./qj_armv5/lib/libx264.a | exit 1
